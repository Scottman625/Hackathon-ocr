...
from train_class import IMAGE_SHAPE_C, IMAGE_SHAPE_H, IMAGE_SHAPE_W, Net, input_define
...
class InferReader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(file_path).convert('RGB')  # 确保图像为 RGB 三通道
        img = img.resize((IMAGE_SHAPE_W, IMAGE_SHAPE_H))  # 调整图像大小
        img = np.array(img, dtype="float32").transpose((2, 0, 1)) / 255  # 转置为 (C, H, W)
        return img
